...
def listar_webhooks_enriquecimento() -> list[dict]:
    """Webhooks de destino salvos pro protótipo de enriquecimento — admin-only."""
    if not eh_admin():
        return []
    sb = _admin_client()
    if not sb:
        return []
    try:
        resp = sb.table("enrichment_webhooks").select("*").order("criado_em").execute()
        return resp.data or []
    except Exception as e:
        logger.error("listar_webhooks_enriquecimento: %s", e)
        return []


def criar_webhook_enriquecimento(nome: str, url: str) -> tuple[bool, str]:
    if not eh_admin():
        return False, "Acesso não autorizado."
    sb = _admin_client()
    if not sb:
        return False, "SUPABASE_SERVICE_ROLE_KEY não configurado."
    try:
        sb.table("enrichment_webhooks").insert({"nome": nome, "url": url}).execute()
        return True, "Webhook salvo."
    except Exception as e:
        logger.error("criar_webhook_enriquecimento: %s", e)
        return False, "Erro ao salvar webhook."


def deletar_webhook_enriquecimento(webhook_id: str) -> tuple[bool, str]:
    if not eh_admin():
        return False, "Acesso não autorizado."
    sb = _admin_client()
    if not sb:
        return False, "SUPABASE_SERVICE_ROLE_KEY não configurado."
    try:
        sb.table("enrichment_webhooks").delete().eq("id", webhook_id).execute()
        return True, "Webhook removido."
    except Exception as e:
        logger.error("deletar_webhook_enriquecimento: %s", e)
        return False, "Erro ao remover webhook."
